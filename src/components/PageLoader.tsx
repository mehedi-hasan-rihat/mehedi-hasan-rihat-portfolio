...
export function PageLoader({ onComplete }: { onComplete?: () => void }) {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      /* ── Phase 1: letters drop in ── */
      tl.to(letters, {
        yPercent: 0, opacity: 1,
        stagger: 0.07, duration: 0.65, ease: "expo.out",
      });

      /* ── Phase 2: role + bar + counter appear ── */
      tl.to(roleRef.current,
        { opacity: 1, yPercent: 0, duration: 0.5, ease: "power3.out" }, "-=0.2");
      tl.to(barTrackRef.current,
        { opacity: 1, scaleX: 1, duration: 0.5, ease: "expo.out" }, "-=0.3");
      tl.to(
      ...
      tl.to(barFillRef.current,
        { scaleX: 1, duration: 1.8, ease: "power2.inOut" }, "+=0.05");
      tl.to(obj, {
      ...
      >
        ...
        <div className="flex flex-col items-center gap-3 w-64 md:w-80">
          <div
            ...
            style={{ opacity: 0 }}
          >
            <div ref={barFillRef} className="absolute inset-y-0 left-0 w-full bg-white origin-left" />
          </div>
