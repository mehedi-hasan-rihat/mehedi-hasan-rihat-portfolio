...
export function Blogs() {
  ...
  useInView(sectionRef, () => {
    ...
    tl.to(items, { opacity: 1, yPercent: 0, stagger: 0.1, duration: 0.8 }, "-=0.5");
  }, { threshold: 0.1 });
        ...
        {["Latest", "Articles"].map((word, i) => (
          <div key={i} className="overflow-hidden">
            <span
              ...
              style={{ fontSize: "clamp(2.8rem, 7.5vw, 7rem)", opacity: 0 }}
            >
              {word}
            </span>
          </div>
      ...
      <div ref={listRef} className="divide-y divide-zinc-800/60 max-w-4xl">
        {site.blogs.map((blog, i) => (
          <a
            ...
            style={{ opacity: 0 }}
          >
            <span className="text-[11px] font-mono text-zinc-700 md:w-10 shrink-0 mt-0.5">{String(i + 1).padStart(2, "0")}</span>
            <div className="flex-1 space-y-2">
              ...
              <h3 className="text-lg md:text-xl font-semibold text-zinc-200 group-hover:text-white transition-colors duration-300 leading-snug">{blog.title}</h3>
              <p className="text-sm text-zinc-600 leading-relaxed line-clamp-2">{blog.excerpt}</p>
            </div>
            <div className="md:text-right shrink-0 flex md:flex-col items-center md:items-end justify-between gap-4">
              <span className="text-[10px] font-mono uppercase tracking-wider border border-zinc-800 px-2.5 py-1 text-zinc-600">{blog.category}</span>
              <svg className="w-4 h-4 text-zinc-700 group-hover:text-white group-hover:translate-x-1 transition-all duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
